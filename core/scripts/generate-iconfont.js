...
const buildfonts = async () => {
    ...
    await svgtofont({
        src: path.resolve(process.cwd(), "src/images/svg/optimized"), // svg path
        dist: path.resolve(process.cwd(), "src/fonts/iconFont/pg-icons"), // output path
        useNameAsUnicode: false,
        fontName: "pg", // font name
        css: {
            output: 'src/scss/core/iconFont',
            cssPath: '../fonts/iconFont/pg-icons/',
            include: 'scss',
            fileName: 'font-icon',
            fontSize: '1.2rem',
        },
